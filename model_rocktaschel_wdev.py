...
class Config(object):
    ...
    tf_saver_folder = "./tf_saver_files/rocktaschel/"
    model_name = "condRNN_with_attention"#"rocktaschel"


class rocktaschel_model():
    ...
    def __init__(self, config):
        ...
        with tf.variable_scope(self.config.model_name):
            ...
            self.target_enc, self.target_last_state = self.add_target_encoder(
                ...
                initial_state=self.source_last_state
                )
...
if __name__ == "__main__":
    ...
    config = Config()
    model = rocktaschel_model(config)

    init = tf.global_variables_initializer()
    with tf.Session() as session_tf:
        # graph for tensorboard
        log_writer = tf.summary.FileWriter('./logs',
                                           session_tf.graph)
        session_tf.run(init)
