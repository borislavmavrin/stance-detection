...
class biRNN_model():
    ...
    def add_first_seq_encoder(self, first_seq_emb, reuse=True):
        ...
        with tf.variable_scope("Target_encoder"):
            ...
            with tf.variable_scope("Backward") as scope:
                if reuse:
                    tf.get_variable_scope().reuse_variables()
                    first_seq_emb_rev = tf.reverse(
                        first_seq_emb, [1])  # reverse the sequence
    ...
    def add_second_seq_encoder(self, last_state_fw, last_state_bw, second_seq_emb):
        ...
        with tf.variable_scope("Tweet_encoder"):
            ...
            with tf.variable_scope("Backward") as scope:
                second_seq_emb_rev = tf.reverse(
                    second_seq_emb, [1])  # reverse the sequence
                cell_fw = tf.contrib.rnn.LSTMCell(
                    self.config.rnn_dim, state_is_tuple=True)
                cell_fw = tf.contrib.rnn.DropoutWrapper(
                    cell=cell_fw, output_keep_prob=self.keep_prob_placeholder)
            ...
            with tf.variable_scope("Witchcraft") as scope:
                ...
                # disappears
                # slices of input are reversed on seq_dim, but only up to
                # seq_lengths
    ...
    def __init__(self, config):
        ...
        """Model."""
        first_seq_emb = self.add_embedding(
            self.first_seq_placeholder, reuse=None)
        second_seq_emb = self.add_embedding(
            self.second_seq_placeholder, reuse=True)
        first_seq_enc, _ = self.add_first_seq_encoder(
            first_seq_emb, reuse=None)
        second_seq_enc, _ = self.add_first_seq_encoder(
            second_seq_emb, reuse=True)

        W = tf.get_variable(
            'feat_manifold_metric',
            shape=[self.config.rnn_dim, self.config.rnn_dim],
            dtype=self.config.float_type_tf,
            initializer=tf.random_normal_initializer()
        )

        thresholds = tf.get_variable(
            'thresholds',
            shape=[self.config.num_classes],
            dtype=self.config.float_type_tf,
            initializer=tf.random_normal_initializer()
        ...
        temp = tf.matmul(tf.squeeze(tf.slice(first_seq_enc, [0, 0, 0], [
                         1, tf.shape(first_seq_enc)[1], self.config.rnn_dim])), W)
        self.features = tf.matmul(tf.squeeze(tf.slice(second_seq_enc, [0, 0, 0], [
                                  1, tf.shape(second_seq_enc)[1], self.config.rnn_dim])), tf.transpose(temp))
        self.logits = tf.matmul(
            tf.reshape(tf.diag_part(self.features), [-1, 1]),
            tf.reshape(thresholds, [1, -1]))
