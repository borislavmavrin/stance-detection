...
class Config(object):
    ...
    keep_prob = 0.9
    debug = False
    train_data_file = "trump_autolabelled_train_stage2.txt"
    dev_data_file = "trump_autolabelled_dev_stage2.txt"
    test_data_file = "SemEval2016-Task6-subtaskB-testdata-gold.txt"
    test_data_stage_2_file = "SemEval2016-Task6-subtaskB-testdata-gold_stage_2.txt"
    ...
    tf_saver_folder = "./tf_saver_files/condBiRNN_stage2/"
    model_name = "condBiRNN_stage2"
...
class biRNN_model():
    ...
    def load_data(self, debug=False):
        ...
        transformed_labels = transform_labels_stage(labels, stage=2)
        ...
        print("Loading test data for stage_2 ...")
        tweets, targets, labels, ids = readTweetsOfficial(
            self.config.test_data_stage_2_file,
            encoding=self.config.defualt_data_file_enc)
        self.tweets_test_stage_2 = tweets
        self.targets_test_stage_2 = targets
        self.labels_test_stage_2 = labels
        self.ids_test_stage_2 = ids
        tweet_tokens = tokenise_tweets(tweets)
        target_tokens = tokenise_tweets(targets)
        transformed_tweets = [transform_tweet(
            w2vmodel, senttoks) for senttoks in tweet_tokens]
        transformed_targets = [transform_tweet(
            w2vmodel, senttoks) for senttoks in target_tokens]
        transformed_labels = transform_labels_stage(labels, stage=2)
        self.encoded_targets_test_stage_2 = np.array(
            transformed_targets, dtype=self.config.int_type_np)
        self.encoded_tweets_test_stage_2 = np.array(
            transformed_tweets, dtype=self.config.int_type_np)
        self.encoded_labels_test_stage_2 = np.array(
            transformed_labels, dtype=self.config.float_type_np)
        self.test_data_size_stage_2 = self.encoded_labels_test_stage_2.shape[0]
    ...
    def add_source_encoder(self, source_emb):
        ...
        with tf.variable_scope("Target_encoder"):
            with tf.variable_scope("Forward") as scope:
                ...
                outputs_fw, last_state_fw = tf.nn.dynamic_rnn(
                    cell=cell_fw,
                    dtype=self.config.float_type_tf,
                    sequence_length=self.source_len_placeholder,
                    inputs=source_emb)

            with tf.variable_scope("Backward") as scope:
                source_emb_rev = tf.reverse(source_emb, [1])  # reverse the sequence
    ...
    def add_target_encoder(self, last_state_fw, last_state_bw, target_emb):
        ...
        with tf.variable_scope("Tweet_encoder"):
            ...
            with tf.variable_scope("Backward") as scope:
                target_emb_rev = tf.reverse(target_emb, [1])  # reverse the sequence
            ...
            with tf.variable_scope("Witchcraft") as scope:
                ...
                outputs_fw = tf.reverse_sequence(
                    outputs_fw_cond, self.target_len_placeholder, seq_dim=1, batch_dim=0)
                ...
                outputs_bw = tf.reverse_sequence(
                    outputs_bw_cond, self.target_len_placeholder, seq_dim=1, batch_dim=0)
                # [batch_size, max_time, cell.output_size]
    ...
    def add_loss_op(self, logits):
        ...
        with tf.variable_scope("Loss"):
            losses = tf.nn.softmax_cross_entropy_with_logits(
                ...
                logits=logits)
            # original paper used tf.reduce_sum
    ...
    def __init__(self, config):
        ...
        with tf.variable_scope(self.config.model_name):
            ...
            """Model."""
            source_emb = self.add_embedding(self.source_placeholder, reuse=None)
            target_emb = self.add_embedding(self.target_placeholder, reuse=True)
            last_state_fw, last_state_bw = self.add_source_encoder(source_emb)
            self.features = self.add_target_encoder(last_state_fw, last_state_bw, target_emb)
